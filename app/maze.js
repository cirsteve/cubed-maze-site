...
function evaluateMove (walls, current, proposed) {
    ...
    var moveMap = {
        ...
        200:()=>walls[current[2]][0][current[0]-1][current[1]],
        //check position south y wall
        110:()=>walls[current[2]][1][current[0]][current[1]-1],
        //check position level up z wall
        101:()=>walls[current[2]-1][2][current[0]][current[1]]

    }
    var delta = parseInt(current.join(''), 10) - parseInt(proposed.join(''), 10) + 100;
    return moveMap[delta] && moveMap[delta]() === '0';
}
...
function* getNeighbors(n) {
    ...
    while (i<progression.length) {
        yield progression[i++](n);
    }
    yield false;
};

var freshNode = function (node, rec) {
    //add untracked node to record map
    //return its first neighbor or false if no valid neighbors exists
    var nodeStr = node.join('');
    var neigbors = getNeighbors(node);
    var neighbor = false;
    var test = null;
    var moves = 6;
    for (var i = 0; i<moves;i++) {
        test = neighbors.next();
        if (evaluateMove(walls, node, test)) {
            rec[nodeStr] = neighbors
            return [record, test]
        }
    }
    return false;
...
function getPath (walls, start, goal) {
    ...
    var path = [start];
    var record = {};
    var nextNode, ultime, ultimteStr, valid;


    do {
        ultimate = path[path.length-1];
        ultimateStr = ultimate.join('');
        if (!record[ultimateStr]) {
            //if ultimate node in path is unvisited add it
            valid = freshNode(ultimate, record);
            if (valid === false) {
                path.shift()
            } else {
                path.unshift(valid)
            ...
            do {
                nextNode = record[ultimateStr].next();
            } while(nextNode && !evaluateMove(walls, ultimate, nextNode))
            if (nextNode) {
        ...
        progress(path);
    } while (!atGoal(path[path.length-1], end))
    return path;
};
